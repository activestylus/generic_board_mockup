...
helpers do
  ...
  def validate(*args)
    p = []
    args.each do |type ,message|
      case type
      when :presence
        p << validate_attr(type, "can't be blank")
      when :email
        p << validate_attr(type, "must be a valid email address")
      end
    end
    p.join(' ')
  end

  def validate_attr(type,message)
    "data-validate-#{type}=\"#{message}\""
  end
